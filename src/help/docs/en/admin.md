...
- **Context menu (right-click)** on Users, Roles, Clouds, and Buckets rows: Copy, Edit, Delete (same shared context-menu styling as the file list). On **Buckets**, also **Access** — manage which users can use the bucket and with which storage role. Edit/Delete may be disabled for protected built-in rows (for example the `admin` role or the `admin` user). On Buckets, Delete is disabled for non-admin.
