...
- **Users** — accounts, passwords, global and per-bucket roles. Admin only.
  - When **adding** or **copying** a user, you can look up a person in AD/LDAP (search icon next to the username): at least 5 characters; partial match on login / display name / email. One match fills the fields immediately; several matches show a selectable list. On copy, permissions and bucket/cloud lists are kept; username / full name / email are set anew.
...
- **Context menu (right-click)** on Users, Roles, Clouds, and Buckets rows: Copy, Edit, Delete (same shared context-menu styling as the file list). On **Buckets**, also **Access** — manage which users can use the bucket and with which `storage_*` role (**Edit Access** / **Add user** / **Apply**). Wildcard (`*`) and `admin` role entries are read-only. Edit/Delete may be disabled for protected built-in rows (for example the `admin` role or the `admin` user). On Buckets, Delete is disabled for non-admin.
